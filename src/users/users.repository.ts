...
export class UserRepository extends Repository<User> {
  async createUser(
    ...
    const { email, usuario, password, pais, nascimento } = createUserDto;
    ...
    user.role = role;
    user.status = true;
    user.confirmacaoToken = crypto.randomBytes(32).toString('hex');
  ...
  async findUsers(
    ...
    const { email, usuario, status, role } = queryDto;
